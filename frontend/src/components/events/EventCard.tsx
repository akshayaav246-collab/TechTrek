...
export function EventCard({ event }: { event: TechEvent }) {
  ...
  return (
    <Card className="flex flex-col h-full border border-border hover:border-primary/50 hover:shadow-xl transition-all duration-300 bg-card group relative overflow-hidden">
      ...
      <div className="text-sm font-medium text-foreground/80 mb-6 flex items-center gap-2 relative z-10">
        <span className="text-primary">📅</span> {formatDate(event.dateTime)}
      </div>
      ...
      <div className="mt-auto pt-6 border-t border-border relative z-10">
        ...
        <Link href={`/events/${event.eventId || event.id}`} className="block w-full">
          <Button variant={event.status === 'UPCOMING' ? 'primary' : 'secondary'} className={`w-full ${event.status === 'COMPLETED' ? 'opacity-80' : ''}`}>
            View Details
          </Button>
        </Link>
      </div>
