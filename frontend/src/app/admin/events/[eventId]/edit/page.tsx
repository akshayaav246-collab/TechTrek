  ...
  return (
    <div>
      ...
      <input type={type} placeholder={placeholder} value={value} required={required}
        ...
        className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#E8831A] transition-colors bg-white" />
    </div>
  );
}
                ...
                {STEPS.map((s, i) => (
                  <div key={i} className="flex items-center flex-1 last:flex-none">
                    <div className={`flex items-center gap-2 ${i + 1 === step ? 'text-[#E8831A]' : i + 1 < step ? 'text-emerald-600' : 'text-gray-300'}`}>
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 cursor-pointer ${i + 1 === step ? 'border-[#E8831A] bg-[#E8831A] text-white' : i + 1 < step ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-gray-200 text-gray-400'}`}
                ...
                {step === 2 && (
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <select className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#E8831A] transition-colors" value={form.status} onChange={e => set('status', e.target.value)}>
